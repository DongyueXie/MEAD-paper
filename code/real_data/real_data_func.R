...
neuron_simu_study = function(indis_ref,
  ...
  for(r in 1:n_rep){
    ...
    if('my'%in%method_list){
      ...
      #                                          groups=groups,
      #                                          calc_var=calc_var)
      fit.err.cor.weight.cv = MEAD_est(y=y,X=X,Vg=V,w=w,
                                       hc.type='cv_indep',
                                       R01=R01,
                                       folds=folds,
                                       groups=groups,
    ...
    if('my_unweighted'%in%method_list){
      ...
      fit.err.cor.weight.cv = MEAD_est(y=y,X=X,Vg=V,w=1,
                                       hc.type='cv_indep',
                                       R01=R01,
                                       folds=folds,
                                       groups=groups,
...
get_coverage_p_sieve = function(p_hat,p_hat_ci_l,p_hat_ci_r,b_array){

  K = dim(p_hat)[1]
  nb = dim(p_hat)[2]
  z = array(dim = dim(p_hat))
  for(i in 1:dim(z)[3]){
    z[,,i] = (b_array[,,i]<=p_hat_ci_r[,,i])*(b_array[,,i]>=p_hat_ci_l[,,i])
  }
  crg = apply(z,c(1,2),function(z){round(mean(z,na.rm=T),3)})
  rownames(crg) = paste('cell',1:K)
  colnames(crg) = paste('bulk',1:nb)
  crg
}

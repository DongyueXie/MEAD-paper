...
sigma2 = xin$sigma2
set.seed(12345)
simu_out = simu_study(ref,sigma2,p=c(0.5,0.3,0.1,0.1),n_bulk = 50,dirichlet.scale=10,
                      printevery = 1,gamma_shape = 5,beta_tilde_transformation = list(method='softplus',a=10))
...
for (k in 1:4) {
  ...
  plot_list[[k]] = ggplot(datax, aes(x = indi, y = true_p)) +
    ...
    xlab("") + # Remove x-axis label from individual plots
    ylab("") +
    geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.3) +
    ggtitle(paste('Cell type:', celltypes[k])) +
    ylim(0, 1)+
    theme(
      axis.text.y = if (k == 1) element_text() else element_blank(),
      axis.ticks.y = if (k == 1) element_line() else element_blank()
    )
}

# Arrange the plots with a single x-axis label
grid.arrange(
  grobs = plot_list,
  ncol = 4,
  widths = c(1.1, 1, 1, 1),
  bottom = textGrob("Sorted bulk samples", gp = gpar(fontsize = 12), vjust = -1.2),
  left = textGrob("Proportions", gp = gpar(fontsize = 12), rot = 90,vjust=1.9,hjust=0.3)
)
